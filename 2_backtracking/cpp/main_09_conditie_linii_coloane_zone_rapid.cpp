...
int unic_pe_coloana(Configuratie *config, int linie, int coloana) {
    for(int i = 0; i < DIM; i++)
...
int unic_pe_zona(Configuratie *config, int linie, int coloana) {
    for(int i = 0; i < DIM; i++)
        for(int j = 0; j < DIM; j++) {
...
void rezolva(Configuratie *config, int linie, int coloana) {
    ...
    // nu vrem sa suprascriem configuratia initiala, presupunem ca e valid
    ...
    for(int val = 1; val <= 9; val++) {
        ...
        // este nevoie sa restauram starea anterioara pentru ca
        // altfel nu mai merg validarile de solutie partiala
        // la revenirea din backtracking, vrem sa incercam alte valori
